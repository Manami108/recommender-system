...
from recall_combined    import recall_combined
...
if __name__ == "__main__":
    ...
    # ─── Stage 1: Extract context signals ───────────────────────────────
    ctx = extract_context(paragraph)
    print("\n— Extracted context signals —")
    for k, v in ctx.items():
        print(f"{k:<18}: {v}")

    # ─── Stage 2: Combined Recall (BM25‐OR + Embed‐NN) ───────────────────
    candidates_df = recall_combined(ctx, limit=60)
    print(f"\nFound {len(candidates_df)} candidate papers.\n")

    if candidates_df.empty:
        print("No candidates found by combined recall.")
    else:
        # Show every column so you can inspect the sim‐score, source, etc.
        pd.set_option("display.max_rows", None)
        pd.set_option("display.max_colwidth", 80)
        print("── All retrieved papers before reranking ──\n")
        print(candidates_df.to_string(index=False))

        # ─── Stage 3: LLM Rerank ────────────────────────────────────────
        best = llm_rerank(paragraph, candidates_df, k=15)
        if best.empty:
            print("\nLLM reranker returned no results.")
        else:
