# eval_norm_llm.py (fixed)
# ---------------------------------------------------------------------------
# Evaluate LLM log-likelihood reranking
# ---------------------------------------------------------------------------
...
def evaluate_case(paragraph: str, true_pids: List[str], target_year: Optional[int]):
    ...
    vec     = recall_vector(embed(cleaned), k=40, sim_threshold=0.30)
    hybrid  = recall_by_chunks(chunks, k_bm25=40, k_vec=40, sim_th=0.30)

    pool = pd.concat([bm25, vec, hybrid], ignore_index=True)
    cand = (
        pool.sort_values(["source","sim"], ascending=[True, False])
            .drop_duplicates("pid", keep="first")
            .reset_index(drop=True)
    )

    meta   = fetch_metadata(cand["pid"].tolist())
    merged = (
        cand.merge(meta[["pid","abstract","year"]], on="pid", how="left")
    ...
    # Log-likelihood rerank
    rer = llm_loglikelihood_rerank(
        paragraph,
    ...
    # sim map & metrics
    cand_absts = merged.set_index("pid").loc[pids, "abstract"].tolist()
    cand_embs  = np.stack([embed(a) for a in cand_absts])
    sims       = cosine_matrix(cand_embs, ref_embs) if n_rel else np.zeros((len(pids),0))
    max_sims   = sims.max(axis=1) if n_rel else np.zeros(len(pids))
    sim_map    = {pid: (s >= SIM_THRESHOLD) for pid, s in zip(pids, max_sims)}
...
def main():
    ...
    print("\nLog-likelihood LLM rerank average metrics:\n")
    print(avg)
