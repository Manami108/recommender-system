...
TESTSET_PATH = Path(os.getenv(
    "TESTSET_PATH",
    "/home/abhi/Desktop/Manami/recommender-system/datasets/testset4.jsonl"))
...
def main() -> None:
    ...
    for prefix in ["P", "HR", "R", "NDCG"]:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(save_dir / f"{prefix.lower()}_rrf_hop3_llm.png", dpi=200)
        plt.close()

    # 3) persist CSV
    csv_dir = Path(__file__).parent / "csv4"
    csv_dir.mkdir(exist_ok=True)
    out_path = csv_dir / "4metrics_rrf_hop3_llm.csv"
