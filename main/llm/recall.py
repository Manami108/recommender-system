...
# DEFAULT_K_BM25   = 40
# DEFAULT_K_VEC    = 40
# DEFAULT_SIM_TH   = 0.30         # cosine similarity threshold (distance→similarity)
# EMBED_DIM        = 768          # SciBERT size; used for fallback
...
#     """Return normalized SciBERT embedding without tqdm."""
...
# def recall_vector(
#     vec: np.ndarray,
#     *, k: int = DEFAULT_K_VEC, sim_threshold: float = DEFAULT_SIM_TH
# ) -> pd.DataFrame:
#     cypher = """
...
#     with dr.session(default_access_mode=READ_ACCESS) as sess:
#         rows = sess.run(
#             cypher, idx=VECTOR_INDEX, k=k,
#             vec=vec.astype(np.float32).tolist(), th=sim_threshold
#         ).data()
#     return pd.DataFrame(rows, columns=["pid","title","year","hop","sim"])
...
#     chunks: Iterable[str],
#     *, k_bm25:int=DEFAULT_K_BM25, k_vec:int=DEFAULT_K_VEC, sim_th:float=DEFAULT_SIM_TH
# ) -> pd.DataFrame:
#     rows:list[dict[str,Any]]=[]
#     for ch in chunks:
#         rows += recall_fulltext(ch,k=k_bm25).assign(source="bm25").to_dict("records")
#         rows += recall_vector(embed(ch),k=k_vec,sim_threshold=sim_th).assign(source="embed").to_dict("records")
#     if not rows:
#         return pd.DataFrame(columns=["pid","title","year","hop","sim","source"])
#     df = pd.DataFrame(rows).sort_values(by=["source","sim"], ascending=[True,False])
#     return df.drop_duplicates(["pid","source"]).reset_index(drop=True)
...
#     return pd.DataFrame(rows,columns=["pid","title","abstract","authors","year"])
