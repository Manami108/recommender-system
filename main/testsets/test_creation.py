...
OUTPUT_CSV = "/media/e-soc-student/DISK2/GR/GR2_Recommendation/datasets/big/testset_300_doi_title.csv"
SAMPLE_SIZE = 300
# =============================

def is_valid_row(row):
    """Check for non-empty title, doi, abstract, and at least 10 references."""
    try:
        # Required fields
...
def main():
    ...
    print(f"Filtered valid entries: {len(filtered_df):,}")

    if len(filtered_df) < SAMPLE_SIZE:
        raise ValueError(f"Only {len(filtered_df)} valid papers found. Cannot sample {SAMPLE_SIZE}.")
